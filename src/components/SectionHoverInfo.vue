    ...
    <div v-else class="section-details">
      ...
      <div class="group-members-section">
        ...
        <div v-else class="group-members-list">
          <template v-if="eventId">
            <div
              v-if="getFriendsForEvent(eventId).length > 0"
              class="group-members-item"
            >
              {{ getFriendsForEvent(eventId).length }}
              {{
                getFriendsForEvent(eventId).length === 1 ? "friend" : "friends"
              }}: {{ getFriendsForEvent(eventId).join(", ") }}
            </div>
            <div v-else class="no-group-members">No friends enrolled</div>
          </template>
        </div>
      </div>
      ...
      <div class="group-members-section">
        ...
        <div v-else class="group-members-list">
          <template
            v-if="eventId"
            v-for="[groupId, members] in getGroupMembersForEvent(eventId)"
            :key="groupId"
          >
          ...
          <div
            v-if="eventId && getGroupMembersForEvent(eventId).length === 0"
            class="no-group-members"
          >
...
const displayCourseName = computed(() => {
  ...
  return colonIndex >= 0
    ? props.courseName.substring(0, colonIndex).trim()
    : props.courseName;
});
...
const displaySectionType = computed(() => {
  ...
  return colonIndex >= 0
    ? props.sectionType.substring(0, colonIndex).trim()
    : props.sectionType;
});
