  ...
  <div class="course-info">
    ...
    <div v-else class="course-details">
      <div class="course-header">
        ...
        <div class="course-header-controls">
            ...
            <div class="preference-buttons">
              <button
                ...
                :class="[
                  ...
                  'preference-likely',
                  {
                    'preference-selected': currentPreference === 2,
                    'preference-disabled': !hasScheduledEvents,
                  },
                ]"
              ...
              <button
                ...
                :class="[
                  ...
                  'preference-maybe',
                  {
                    'preference-selected': currentPreference === 1,
                    'preference-disabled': !hasScheduledEvents,
                  },
                ]"
              ...
              <button
                ...
                :class="[
                  ...
                  'preference-unlikely',
                  {
                    'preference-selected': currentPreference === 0,
                    'preference-disabled': !hasScheduledEvents,
                  },
                ]"
          ...
          <div class="group-filter-section">
            <label for="group-filter" class="group-filter-label"
              >Show members of group:</label
            >
            <select
              id="group-filter"
      ...
      <div
        ...
        :style="{
          '--grid-columns': eventTypeColumns.length,
          gridTemplateColumns: `repeat(${eventTypeColumns.length}, minmax(150px, 1fr))`,
        }"
      >
        <div
        ...
        >
          ...
          <ul v-else class="events-list">
            <li
              ...
              :class="[
                'event-item',
                { 'event-scheduled': isEventScheduled(event.event) },
              ]"
              @click="handleEventClick(event)"
            >
              <div
                class="radio-button"
                :class="{ 'radio-selected': isEventScheduled(event.event) }"
              ></div>
              <div class="event-content">
                <div class="event-days">{{ formatDays(event.times.days) }}</div>
                <div class="event-time">
                  {{
                    formatTimeRange(event.times.startTime, event.times.endTime)
                  }}
                </div>
                ...
                <div v-else class="event-friends">
                  <span v-if="getFriendsForEvent(event.event).length > 0">
                    {{ getFriendsForEvent(event.event).length }}
                    {{
                      getFriendsForEvent(event.event).length === 1
                        ? "friend"
                        : "friends"
                    }}: {{ getFriendsForEvent(event.event).join(", ") }}
                  </span>
                  <span v-else class="no-friends"> No friends enrolled </span>
                </div>
                ...
                <div v-else class="event-friends">
                  <template
                    v-for="[groupId, members] in getGroupMembersForEvent(
                      event.event
                    )"
                    :key="groupId"
                  >
                    <div v-if="members.length > 0" class="event-group-members">
                      Group {{ getGroupNameById(groupId) }}:
                      {{ members.join(", ") }}
                    </div>
                  </template>
                </div>
...
import {
  getEventFriends,
  type EventFriendsResult,
} from "@/api/syncs/friendsInEvents";
import {
  getUserGroups,
  getMembersInEvents,
  getGroupName,
...
const getCourseCode = (courseName: string): string => {
  ...
  return colonIndex >= 0
    ? courseName.substring(0, colonIndex).trim()
    : courseName;
};
...
const hasScheduledEvents = computed(() => {
  ...
  return props.course.events.some((event) =>
...
const groupMembersByEventId = ref<Map<string, Map<string, string[]>>>(
  new Map()
);
...
const scheduledEventsByType = computed(() => {
  if (!props.course || !props.scheduledEventIds)
